...
from mqtt_config import mqTopic, deviceID, audioFileDirectory
...
def publish(client, fileName, audioText):
...
def run(paramFilepath, paramAudioText):
    client = connect_mqtt()
    client.loop_start()
    try:
        publish(client)
